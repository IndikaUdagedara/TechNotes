...
- An area of a document that is laid out using flexbox is called a ==**flex
  container**==. To create
  a [flex container](https://developer.mozilla.org/en-US/docs/Glossary/Flex_Container),
  set the
  area's [`display`](https://developer.mozilla.org/en-US/docs/Web/CSS/display) property
  to `flex`.
- In a flex container, the direct children of that container become ==**flex
  items**==.
- align-items: This defines the default behavior for how flex items are laid out
